defmodule Signet.RPC do
  ...
  def execute_trx(contract, call_data, opts \\ []) do
    ...
    trx_type_result = case {trx_type, gas_price_user, priority_fee, base_fee_user} do
      {ty, gas_price, nil, nil} when (ty == :v1 or is_nil(ty)) and not is_nil(gas_price) ->
        {:ok, {:v1, to_wei(gas_price)}}

      {ty, nil, max_priority_fee_per_gas, max_fee_per_gas}
      when (ty == :v2 or is_nil(ty)) and not is_nil(max_priority_fee_per_gas) and
             not is_nil(max_fee_per_gas) ->
      ...
      {ty, nil, max_priority_fee_per_gas_user, nil} when is_nil(ty) or ty == :v2 ->
        max_priority_fee_per_gas = (if is_nil(max_priority_fee_per_gas_user), do: 0, else: to_wei(max_priority_fee_per_gas_user))
