    ...
    for {test_info, i} <- Enum.with_index(@tests) do
      ...
      test test_info[:name] do
        ...
        if t[:exp_error] do
          ...
          if t[:exp_return] do
            assert not execution_result.reverted, "Expected success, got revert #{inspect(execution_result)}"
            assert to_hex(t[:exp_return]) == to_hex(execution_result.return_data)
          end

          if t[:exp_revert] do
            assert execution_result.reverted, "Expected revert, got success #{inspect(execution_result)}"
